...
> **状态：历史记录，勿据此判断当前代码（2026-09-10 复核）**
>
> 本报告列出的问题绝大多数已在后续提交中修复：P0-1 GDI 刷子泄漏/颜色失效、P0-2 INI 竞态加锁、
> P0-3 SQL 注入（白名单 + 临时 .sql 文件）、P1-4 env block、P1-5 下载线程 cancel 共享指针、
> P1-6 日志回投 UI 线程、P1-7 nginx 进程归属校验、P1-8 redis 测试实例回收、P1-9 可选 SHA256 校验、
> P1-10 DPAPI 口令加密、P1-11 优雅退出轮询，以及 P2 中的进度溢出、原子写 ini、UNC makeDirs、
> `wstrfmt` 改名、`/MT /W4`、packages.conf.example、gitignore 规则等。
>
> 保留本文件仅作为「问题来源 + 设计取舍」的历史记录；其中未提到的 P1/P2 条目已逐条核对过代码，
> 请以当前源码为准（2026-09-10 又修了一轮：退出流程、管道句柄继承、进程句柄泄漏、vhost 回滚、
> 明文口令迁移、exeDir 竞态、日志轮转等）。
